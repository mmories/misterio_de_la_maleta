...
import {singingText,MISSION,MISSION_OBJECTIVE,PAPER_INFO,MASTER_KEY_DIALOGUE,LIFT_REQUIREMENTS,EXPLORATION_REWARD_PERCENT} from './content.js';
...
const LOBBY_WALK_FRAME=[0,1,2,3,4,5,6,7];
let state=newState(),scene='title',verb='MIRAR',selected=null,hover=null,busy=false,epoch=0,last=performance.now(),clock=0;
let player={x:229,y:551,dir:0,visible:false,pose:null},path=[],moveResolve=null,speed=135,gait=0,moveEnergy=0,moveMarker=null;
...
let speechResolve=null,typing=null,fullLine='',typed=0,speechTimer=null,toastTimer=null,objectiveTimer=null,lastPicked=null,pickTimer=null;
...
function showMissionDetails(){if(busy)return;const progress=missionStats();const lift=LIFT_REQUIREMENTS.map(id=>({id,label:itemName(id),done:id==='key'?state.hasKey310:id==='rulesBook'?state.hasRulesBook:id==='tobacco'?state.hasTobacco:state.hasEmpiLetter}));modal(`<div class="mission-report"><small>MISIÓN 0 · LA LLEGADA</small><h2>La llegada</h2><section class="mission-objective"><small>OBJETIVO</small><p>${MISSION_OBJECTIVE}</p></section><h3>PARA SUBIR A LA 310</h3>${lift.map(r=>`<p class="requirement ${r.done?'done':''}">${r.done?'✓':'○'} ${r.label.toUpperCase()}</p>`).join('')}<h3>SECRETOS OPCIONALES</h3><div class="report-score">${progress.percent}%</div>${progress.rows.map(r=>`<p class="progress-row"><span>${r.label}</span><b>${r.done} / ${r.total}</b><i><em style="width:${r.done/r.total*100}%"></em></i></p>`).join('')}<small>Los cuatro requisitos de arriba bastan para avanzar. Las sorpresas se desbloquean desde el ${EXPLORATION_REWARD_PERCENT}% y el 100% sigue siendo posible al volver a recepción.</small></div>`);}
function inventoryItems(){return ['bag',...(state.hasMasterKey?['masterKey']:[]),...(state.hasKey310?['key']:[]),...(state.hasUmbrella?['umbrella']:[]),...state.tookPapers,...(state.hasComerciaNote?['comerciaNote']:[]),...(state.hasTobacco?['tobacco']:[]),...(state.hasEmpiLetter?['empiLetter']:[]),...(state.hasRulesBook?['rulesBook']:[])];}
function inventoryIcon(id){
 if(id==='tobacco'||id==='masterKey'){const icon=document.createElement('img');icon.src='assets/'+(id==='tobacco'?'fortuna.svg':'master-key.svg');icon.width=64;icon.height=54;icon.alt='';icon.setAttribute('aria-hidden','true');return icon;}
 if(id==='umbrella'){const icon=document.createElement('span');icon.className='umbrella-icon';icon.setAttribute('aria-hidden','true');Object.assign(icon.style,{backgroundImage:"url('assets/umbrella-stand-deusto-v1.png')",backgroundRepeat:'no-repeat',backgroundSize:'71px 106px',backgroundPosition:'-21px -3px'});return icon;}
 const icon=document.createElement('span');icon.className='symbol';
 icon.textContent=(id==='key'||id==='masterKey')?'⚿':id==='umbrella'?'☂':id==='bag'?'▣':id==='comerciaNote'?'✦':id==='empiLetter'?'✉':id==='tobacco'?'▥':id==='rulesBook'?'▧':'▤';
 if(images.walk&&images.actions&&!['comerciaNote','tobacco','empiLetter','rulesBook','masterKey','umbrella'].includes(id)){
  const ic=document.createElement('canvas');ic.width=64;ic.height=54;ic.setAttribute('aria-hidden','true');const ig=ic.getContext('2d');ig.imageSmoothingEnabled=false;
  if(id==='bag')ig.drawImage(frames.walk[0],frames.walk[0].width*.59,frames.walk[0].height*.56,frames.walk[0].width*.41,frames.walk[0].height*.36,17,3,32,48);
  else if(id==='key')ig.drawImage(frames.actions[6],0,45,28,65,21,0,23,54);
  else{const h=HOTSPOTS.find(h=>h.id===id),[x,y,w,hh]=h.rect;ig.drawImage(images.reception,x/960*1448,y/600*1086,w/960*1448,hh/600*1086,4,10,56,35);}return ic;
 }
 return icon;
}
function makeInventoryButton(id){const b=document.createElement('button');b.type='button';b.dataset.item=id;b.className=(selected===id?'selected ':'')+(lastPicked===id?'just-picked':'');b.setAttribute('role','listitem');b.setAttribute('aria-label',itemName(id)+(selected===id?', seleccionado':''));b.append(inventoryIcon(id));const label=document.createElement('span');label.textContent=itemName(id);b.append(label);if(selected===id){const mark=document.createElement('small');mark.textContent='SELECCIONADO';b.append(mark);}b.onclick=()=>{if($('inventory-modal').open)$('inventory-modal').close();inventoryClick(id);};return b;}
...
function walk(to,fast=false,direct=false){if(moveResolve){moveResolve(false);moveResolve=null;}path=direct?[to]:findPath([player.x,player.y],to)||[];speed=fast?310:135;moveMarker={x:to[0],y:to[1]};if(!path.length){moveEnergy=0;moveMarker=null;return Promise.resolve(false);}player.pose=null;return new Promise(r=>moveResolve=r);}
...
function renderFrameWidth(sprite,x,y,width,flip=false){if(!sprite)return;width=Math.round(width);const height=Math.round(sprite.height/sprite.width*width);ctx.save();ctx.translate(Math.round(x),Math.round(y));if(flip)ctx.scale(-1,1);ctx.drawImage(sprite,Math.round(-width/2),-height,width,height);ctx.restore();}
...
function renderPlayer(){
 ...
 const entranceWalk=scene==='exterior'&&['exit','threshold','entering'].includes(introStage);
 const scale=playerScale(),height=154*scale,moving=path.length>0||(entranceWalk&&player.pose===null);
 ...
 if(player.farewell&&scene==='exterior')f=frames.actions[13];
 else if(iso&&player.pickupFrame!==undefined)f=frames.isoPickup[(isoRow===1||isoRow===3?3:0)+player.pickupFrame];
 else if(iso&&player.pose===null){
  if(moving)f=frames.isoWalk[isoRow*8+LOBBY_WALK_FRAME[motion.frame]];
 ...
 ctx.scale(1,1+pulse);
 // Close only the upper trouser seam. Extending this shape toward the shoes
 // would read as an extra pair of legs on the widest walking frames.
 if(iso&&moving){const seamTop=-height*.405,seamBottom=-height*.31,seamWidth=height*.027;ctx.fillStyle='#173d70';ctx.beginPath();ctx.moveTo(-seamWidth,seamTop);ctx.lineTo(seamWidth,seamTop);ctx.lineTo(seamWidth*.62,seamBottom);ctx.lineTo(-seamWidth*.62,seamBottom);ctx.closePath();ctx.fill();}
...
function renderPrim(){if(frames.primIdle){const near=player.visible&&Math.hypot(player.x-269,player.y-442)<145,alert=primAlert>clock||(near&&path.length>0),phase=alert?4+Math.floor(clock*4.5)%4:idleFrame(clock,4,.8),f=frames.primIdle[phase],w=132;ctx.save();ctx.fillStyle='rgba(7,12,13,.26)';ctx.beginPath();ctx.ellipse(269,443,w*.37,3.5,0,0,Math.PI*2);ctx.fill();ctx.restore();renderFrameWidth(f,269,443,w);}else if(images.prim){const cellW=images.prim.width/2;ctx.drawImage(images.prim,primAlert>clock?cellW:0,40,cellW,180,205,355,128,76+breath*.4);}}
function renderLobbyActors(){
 const layers=[{y:player.y,draw:renderPlayer},{y:443,draw:renderPrim},{y:394,draw:renderSenior},{y:545,draw:drawUmbrellaStand},...LOBBY_OCCLUDERS.map(({y,polygon})=>({y,draw:()=>drawOccluder(polygon)}))];
...
function drawUmbrellaStand(){
 if(images.umbrella){const sx=280,sy=state.hasUmbrella?710:60,sw=500,sh=state.hasUmbrella?740:1420,h=state.hasUmbrella?98:160,w=Math.round(sw/sh*h);ctx.drawImage(images.umbrella,sx,sy,sw,sh,740-w/2,545-h,w,h);return;}
 ctx.save();ctx.translate(740,537);ctx.fillStyle='rgba(4,8,8,.25)';ctx.beginPath();ctx.ellipse(0,8,19,3,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#b99153';ctx.fillStyle='#60442b';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-13,-2);ctx.lineTo(-9,7);ctx.lineTo(9,7);ctx.lineTo(13,-2);ctx.closePath();ctx.fill();ctx.stroke();if(state.hasUmbrella){ctx.restore();return;}ctx.strokeStyle='#191f22';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(0,-2);ctx.lineTo(0,-56);ctx.stroke();ctx.beginPath();ctx.arc(0,-58,5,Math.PI,0);ctx.stroke();ctx.fillStyle='#263138';ctx.beginPath();ctx.moveTo(-14,-51);ctx.lineTo(0,-71);ctx.lineTo(14,-51);ctx.closePath();ctx.fill();ctx.strokeStyle='#9eb3b0';ctx.lineWidth=1;ctx.stroke();ctx.restore();
}
...
function preparePassat(img){
 ...
 const visit=i=>{if(i<0||i>=w*h||seen[i])return;seen[i]=1;const k=i*4;
  if(d[k+3]&&!(Math.min(d[k],d[k+1],d[k+2])>172&&Math.max(d[k],d[k+1],d[k+2])-Math.min(d[k],d[k+1],d[k+2])<22))return;
  d[k+3]=0;queue.push(i);
 };
 for(let x=0;x<w;x++){visit(x);visit((h-1)*w+x);}
 for(let y=0;y<h;y++){visit(y*w);visit(y*w+w-1);visit(y*w+w/2);}
 for(let n=0;n<queue.length;n++){const i=queue[n],x=i%w;
  for(const dy of [-1,0,1])for(const dx of [-1,0,1])if(x+dx>=0&&x+dx<w)visit(i+dy*w+dx);
 }
...
function drawCMDEntrance(){
 ...
 ctx.transform(1,cmdDoor*.18,0,1,x,y);
 ctx.drawImage(images.exterior,x/W*images.exterior.width,y/H*images.exterior.height,w/W*images.exterior.width,h/H*images.exterior.height,0,0,leafWidth,h);
 ctx.strokeStyle='#4a3e2a';ctx.lineWidth=1;ctx.strokeRect(0,0,leafWidth,h);
...
function drawExteriorPlayer(){
 ...
 if(['entering','inside-cmd'].includes(introStage))drawCMDEntranceOcclusion();
 if(player.y<car.y-70&&images.passat)drawPassat();
 drawCarDoor();
}
function drawCMDEntranceOcclusion(){
 // Restore the painted jamb and lintel over Julito. The central rectangle is
 // left open, so his body passes through the doorway instead of over the wall.
 const x=482,y=368,w=48,h=64;
 ctx.save();ctx.beginPath();ctx.rect(x,y,w,h);ctx.rect(495,383,20,40);ctx.clip('evenodd');
 ctx.drawImage(images.exterior,0,0,W,H);ctx.restore();
}
...
if(path.length){
 ...
 if(!path.length&&moveResolve){const resolve=moveResolve;moveResolve=null;resolve(!movement.blocked);}
}else moveEnergy=0;
advanceThrownObject();
...
drawRulesOnFloor();drawMoveMarker();renderLobbyActors();drawThrownObject();}
...
function uniformFrameBounds(source){const width=Math.max(...source.map(frame=>frame.width)),height=Math.max(...source.map(frame=>frame.height));return source.map(frame=>{const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;canvas.getContext('2d').drawImage(frame,Math.round((width-frame.width)/2),height-frame.height);return canvas;});}
...
async function intro(){
 ...
 try{
  ...
  // A held side pose establishes that he has stepped down before walking; it
  // avoids the old crouched sprite sliding out through the car body.
  player.visible=true;player.x=430;player.y=555;player.dir=2;player.pose=7;
  await sleep(420);assertEpoch(e);audio.effect('case');
  await animateIntro(.48,t=>{const s=t*t*(3-2*t);player.x=430+38*s;player.y=555-18*s;player.pose=t<.18?7:null;gait=t*Math.PI;},e);
  player.pose=7;await sleep(220);assertEpoch(e);player.pose=null;
  // Step fully clear of the open leaf before it starts to close.
  await animateIntro(.55,t=>{const s=t*t*(3-2*t);player.x=470+35*s;player.y=535-30*s;gait+=t*.16;},e);
  introStage='door-closing';await animateIntro(.7,t=>car.door=1-t*t*(3-2*t),e);audio.effect('door');
  await sleep(300);assertEpoch(e);introStage='farewell';player.dir=3;player.farewell=true;
  ...
  introStage='approach';await walkRoute([[505,475],[503,445],[503,423]]);assertEpoch(e);
  introStage='opening-cmd';player.dir=1;audio.effect('door');
  await animateIntro(.55,t=>cmdDoor=t,e);
  introStage='entering';
  // Use the regular locomotion system for the last steps. The foot anchor,
  // perspective and cadence now advance together instead of tweening upward.
  await walkRoute([[505,416],[505,405]]);assertEpoch(e);
  introStage='inside-cmd';
  await animateIntro(.14,t=>entryAlpha=1-t,e);player.visible=false;
...
function showChoices(){
 busy=true;const choiceEpoch=epoch;let choosing=false;
 ...
 for(const [id,label] of options){const b=document.createElement('button');b.textContent=(state.readTopics.includes(id)?'· ':'')+label;b.onclick=async event=>{
  event?.stopPropagation?.();if(choosing||choiceEpoch!==epoch||scene!=='lobby'||$('choices').hidden)return;choosing=true;
  $('choices').hidden=true;
  try{
  ...
  if(id==='suitcase'){primAlert=clock+5;if(state.hasEmpiLetter)await lines([['Julito','Empi escribió que la maleta había vuelto. Usted también lo ha dicho.'],['Pedro','Entonces ya sois dos los que deberíais hablar más bajo.']]);}updateMissionMeter();showChoices();
   ...
   console.error('Error en una respuesta de Pedro:',err);cancelSpeech();player.pose=null;pedroPose=3;
   toast('Pedro ha perdido el hilo. Puedes elegir otro tema.');
   if(choiceEpoch===epoch&&scene==='lobby')showChoices();
  }finally{if(choiceEpoch===epoch&&scene==='lobby'&&id!=='bye'&&$('choices').hidden)showChoices();}
 };$('choices').append(b);}
 for(const b of $('choices').children){const treated=b.textContent.startsWith('· ');b.classList.toggle('treated-topic',treated);b.classList.toggle('new-topic',!treated&&b.textContent!=='Eso es todo. Gracias, Pedro.');}$('choices').scrollTop=0;$('choices').hidden=false;
}
...
if(h.id==='umbrellaStand'&&verb==='COGER'){if(state.hasUmbrella){await speak('El paragüero sigue aquí, pero el paraguas ya lo llevo conmigo.');return;}await pickupMotion(true);audio.effect('item');state.hasUmbrella=true;remember(state.collectedItems,'umbrella');refreshSceneObjects();refreshInventory();toast('Has cogido: PARAGUAS AZUL DEUSTO');await speak('Un paraguas azul Deusto. En Bilbao no es un accesorio: es una declaración de intenciones. Me lo guardo para cuando alguien proponga un duelo con mala previsión.');player.pose=null;return;}
...
async function rewardMasterKey(){
 if(state.rewardShown||missionStats().percent<EXPLORATION_REWARD_PERCENT)return;
...
$('canvas').onclick=async e=>{if(scene!=='lobby'||(busy&&!path.length))return;const r=canvas.getBoundingClientRect(),to=[(e.clientX-r.left)/r.width*W,(e.clientY-r.top)/r.height*H];if(pointInPolygon(to)){if(moveResolve){moveResolve(false);moveResolve=null;}path=[];activeInteraction=null;busy=false;selected=null;refreshInventory();setSentence();await walk(to);await maybeReward();}};
...
async function load(){try{Object.assign(images,await loadImages());images.passat=preparePassat(images.passat);frames.isoWalk=isometricFrames(images.isoWalk,8,4);frames.isoIdle=isometricFrames(images.isoIdle,2,4);frames.isoPickup=isometricFrames(images.isoPickup,3,2);frames.isoTalk=isometricFrames(images.isoTalk,2,2);frames.walk=anchorJulitoFrames(spriteFrames(images.walk));frames.walkSmooth=anchorJulitoFrames(transparentGridFrames(images.walkSmooth,8,3));frames.actions=anchorJulitoFrames(spriteFrames(images.actions));frames.pedro=spriteFrames(images.pedro);frames.pedroIdle=transparentGridFrames(images.pedroIdle,8,1);frames.primIdle=uniformFrameBounds(transparentGridFrames(images.primIdle,8,1,{cutoff:188,harden:false}));frames.collegiala=transparentSingleSprite(images.colegiala);setHotspots();refreshInventory();$('hotspots').hidden=true;$('start').disabled=false;$('start').textContent='NUEVA PARTIDA';refreshContinue();requestAnimationFrame(()=>{fitToViewport();requestAnimationFrame(draw);});}catch(err){$('start').textContent='REINTENTAR';$('start').disabled=false;$('start').onclick=()=>location.reload();$('sentence').textContent=err.message+'. Pulsa reintentar.';console.error(err);}}
fitToViewport();
load();
