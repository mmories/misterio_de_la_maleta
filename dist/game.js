...
function prepareUmbrellaSprite(){const umbrellaSprite=new Image();umbrellaSprite.onload=()=>{images.umbrella=umbrellaSprite;};umbrellaSprite.src='assets/umbrella-stand-deusto-v1.png';}
...
function refreshSceneObjects(){$('hotspot-rulesBook').hidden=!state.rulesOnFloor||state.hasRulesBook;$('hotspot-umbrellaStand').hidden=state.hasUmbrella;}
...
function renderLobbyActors(){
 const layers=[{y:player.y,draw:renderPlayer},{y:443,draw:renderPrim},{y:394,draw:renderSenior},...(!state.hasUmbrella?[{y:545,draw:drawUmbrellaStand}]:[]),...LOBBY_OCCLUDERS.map(({y,polygon})=>({y,draw:()=>drawOccluder(polygon)}))];
...
function drawUmbrellaStand(){
 ...
 if(images.umbrella){const sx=280,sy=60,sw=500,sh=1420,h=160,w=Math.round(sw/sh*h);ctx.drawImage(images.umbrella,sx,sy,sw,sh,740-w/2,545-h,w,h);return;}
 ctx.save();ctx.translate(740,537);ctx.fillStyle='rgba(4,8,8,.25)';ctx.beginPath();ctx.ellipse(0,8,19,3,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#b99153';ctx.fillStyle='#60442b';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-13,-2);ctx.lineTo(-9,7);ctx.lineTo(9,7);ctx.lineTo(13,-2);ctx.closePath();ctx.fill();ctx.stroke();ctx.strokeStyle='#191f22';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(0,-2);ctx.lineTo(0,-56);ctx.stroke();ctx.beginPath();ctx.arc(0,-58,5,Math.PI,0);ctx.stroke();ctx.fillStyle='#263138';ctx.beginPath();ctx.moveTo(-14,-51);ctx.lineTo(0,-71);ctx.lineTo(14,-51);ctx.closePath();ctx.fill();ctx.strokeStyle='#9eb3b0';ctx.lineWidth=1;ctx.stroke();ctx.restore();
}
...
async function intro(){
 ...
 try{
  ...
  introStage='entering';
  // Cross the threshold while the CMD door remains fully open. Fade only once
  // Julito is behind the glass, instead of clipping his body through the leaf.
  await animateIntro(.85,t=>{const s=t*t*(3-2*t);player.y=423-30*s;entryAlpha=t<.72?1:Math.max(0,(1-t)/.28);gait+=t*.12;},e);player.visible=false;
...
function showPaper(id){const paper=PAPER_INFO[id],card=$('card');if(id==='umbrella'){card.innerHTML='<article class="umbrella-view"><small>PERTENENCIA DE JULITO</small><h2>PARAGUAS AZUL DEUSTO</h2><p>Compacto, con mango de madera y una funda que ha conocido mejores octubres.</p><p>En Bilbao parece más una herramienta que una prenda. Mejor guardarlo para la lluvia… o para un duelo educado.</p><button>GUARDAR EL PARAGUAS</button></article>';}else if(paper)card.innerHTML=`<article class="newspaper-view"><img src="assets/${paper.image}" alt="Portada de ${paper.name}, octubre de 1994"><div><small>ESPAÑA · OCTUBRE DE 1994</small><h2>${paper.name}</h2><h3>${paper.title}</h3><p>${paper.subtitle}</p></div><button>PLEGAR EL PERIÓDICO</button></article>`;else{const info=['LA COMERCIAL','INVITACIÓN RESERVADA','Reunión a las 19:00. Corbata opcional; hablar de mercados, inevitable.','«La élite de los alumnos de Deusto». La modestia no ha pasado el filtro.'];card.innerHTML=`<small>OCTUBRE DE 1994</small><h2>${info[0]}</h2><h3>${info[1]}</h3><p>${info[2]}</p><hr><p>${info[3]}</p><button>DEJAR DE LEER</button>`;}card.hidden=false;audio.effect(id==='umbrella'?'item':'paper');return new Promise(r=>{let closed=false;const close=()=>{if(closed)return;closed=true;audio.effect(id==='umbrella'?'item':'paper');card.hidden=true;card.onclick=null;r();};card.querySelector('button').onclick=e=>{e?.stopPropagation?.();close();};card.onclick=close;});}
