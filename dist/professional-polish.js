...
function toast(text){const el=$('toast');if(!el)return;el.textContent=text;el.hidden=false;clearTimeout(window.__polishToast);window.__polishToast=setTimeout(()=>el.hidden=true,2600);}
...
function ensureProgressHud(){
 ...
 b.addEventListener('click',()=>{
  ...
  modal(`<div class="mission-report professional-report"><small>MISIÓN 0 · LA LLEGADA</small><h2>Progreso del episodio</h2><section class="mission-objective"><small>PARA AVANZAR</small><p>Estos requisitos desbloquean el ascensor. No forman parte del reto de completar el 100%.</p></section><div class="required-grid">${r.rows.map(x=>`<p class="required-row ${x.done?'done':''}"><span>${x.done?'✓':'○'}</span><b>${x.label}</b><em>${x.done?'LISTO':'PENDIENTE'}</em></p>`).join('')}</div><section class="completion-block"><small>EXPLORACIÓN OPCIONAL</small><div class="report-score">${p}%</div><p>${p===100?'Recepción completada al 100%. Has agotado las interacciones contabilizadas.':'Puedes terminar el episodio sin llegar al 100%. El porcentaje premia mirar, conversar y probar interacciones opcionales.'}</p></section></div>`);
 });
 host.append(b);
}
function updateProgressHud(){
 ...
 hud.classList.toggle('ready',r.done===r.total);hud.classList.toggle('complete',p===100);
 // The original meter remains the source of truth for exploration, but its wording is made explicit.
...
function ensureUmbrella(){
 ...
 b.addEventListener('click',e=>{e.stopPropagation();const verb=activeVerb();if(hasUmbrella()){
  ...
  if(verb==='COGER'){
   ...
   modal('<small>OBJETO CONSEGUIDO</small><h2>Paraguas</h2><p>Negro, largo y sorprendentemente sólido.</p><p><b>Julito:</b> «Puede servir para la lluvia… o para un duelo a muerte. En Bilbao conviene estar preparado para ambas cosas.»</p>');
...
function syncInventory(){
 ...
 const b=document.createElement('button');b.type='button';b.dataset.polishItem='umbrella';b.setAttribute('role','listitem');b.setAttribute('aria-label','paraguas');b.innerHTML='<span class="symbol umbrella-symbol">☂</span><span>paraguas</span><small>PARA MÁS ADELANTE</small>';
